...
(function () {
  ...
  window.web3wrap = web3;
  window.loadJsonRes = path => new Promise(resolve => {
    RES.getResByUrl(path, resolve, null, 'json');
  });

  let addresses;

  window.getContractInstance = async (name, path) => {
    addresses = addresses || await loadJsonRes('contracts/build/contract_addresses.json');
    const className = name.substring(0, 1).toUpperCase() + name.substring(1);

    path = path || `contracts/build/contracts/${className}.json`;
    const data = await loadJsonRes(path);

    return web3wrap.loadContract(data.abi, addresses[name]);
  };
})();
